...
func BundleModules(opts BundleOptions, mods []string, write bool) ([]BuildContextResult, error) {
	...
	plugins := makePlugins(mods, commonMods, absModsDir, logger)
...
func makeModuleSet(mods, commonMods []string) (externalMods *set.Set[string], modPaths []string) {
	externalMods = set.New[string](len(mods) + len(commonMods) + 1)

	// generated by the server, so it's always an external path
	externalMods.Insert("./eval.js")
...
func makePlugins(mods, commonMods []string, wrkdir string, logger *slog.Logger) (res [][]esbuild.Plugin) {
	externalMods, modPaths := makeModuleSet(mods, commonMods)
	for i := range mods {
		plugin := makePlugin(modPaths[i], externalMods, wrkdir, logger)
		res = append(res, []esbuild.Plugin{plugin})
	}
	return res
}

func makePlugin(mod string, externalMods *set.Set[string], wrkdir string, logger *slog.Logger) esbuild.Plugin {
		...
		Setup: func(build esbuild.PluginBuild) {
			...
			build.OnResolve(esbuild.OnResolveOptions{Filter: `^(./|../)`},
				func(args esbuild.OnResolveArgs) (esbuild.OnResolveResult, error) {
					...
					logger.Debug("Checking import", "from", args.Importer, "path", args.Path, "isExternal", external)
					if external {
